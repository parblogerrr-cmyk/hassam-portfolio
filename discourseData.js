const discourseWritings = [
    {
        id: 1,
        title: "Introduction to Discourse: The Breath of the Mountains",
        image: "mountain_discourse_1766764024919.png",
        content: `
            <p>Discourse is more than just words; it is the "living breath" of a culture. In the Northern Areas of Pakistan, discourse isn't found only in textbooks but in the way a village elder recounts a folk tale in Shina or how a youth in Gilgit navigates a conversation in Urdu. For this portfolio, I define discourse as the social glue that holds our mountainous communities together. It is the study of how language reflects our high-altitude reality.</p>
            <p>By analyzing these "texts" of daily life, we uncover the values, fears, and hopes of the North. This entry serves as the foundation, looking at the North not as a silent landscape, but as a vibrant, speaking entity where every dialogue is a window into a complex social world.</p>
        `
    },
    {
        id: 2,
        title: "Language and Society: The Multilingual High-Road",
        image: "multilingual_road_1766764060824.png",
        content: `
            <p>In the valleys of Gilgit-Baltistan and Chitral, society is inherently multilingual. A single person might use Burushaski at home, Shina in the bazaar, and Urdu or English for education. This sociolinguistic layering is a survival strategy and a mark of identity. Using the "Language and Society" framework, I examine how code-switching in the North isn't just a linguistic habit—it’s a social map.</p>
            <p>Who speaks what, and to whom? The prestige of English often clashes with the warmth of the mother tongue. This writing explores the social hierarchy of languages along the Karakoram Highway, where the choice of a single word can signal one’s village, education level, and social standing in a rapidly changing world.</p>
        `
    },
    {
        id: 3,
        title: "Discourse and Identity: The Borderland Self",
        image: "mountain_discourse_1766764024919.png",
        content: `
            <p>One of the most striking lessons of Discourse Studies is that identity is not fixed—it is performed through language. This idea became clearer when I reflected on multilingual communities in places like Hunza Valley.</p>
            <p>A speaker in Hunza may use Burushaski to express intimacy and belonging, Urdu to engage with national identity, and English to signal education or global awareness. Each language activates a different version of the self. Discourse, therefore, becomes a tool for negotiating identity rather than merely expressing it.</p>
            <p>This course helped me realize that identity in the north is layered, not divided. People do not choose between being local or national; they shift identities through discourse depending on context. Multilingualism allows them to exist at cultural borders without losing themselves.</p>
        `
    },
    {
        id: 4,
        title: "History of Discourse Analysis: Tracking the Narrative",
        image: "multilingual_road_1766764060824.png",
        content: `
            <p>The history of Discourse Analysis (DA) has moved from analyzing sentences to analyzing the world. Just as the ancient Silk Road evolved from a narrow path to a modern highway, DA has expanded its reach. In this reflection, I trace how the field moved from the formal linguistics of the West to the "Social Turn" that allows us to study indigenous contexts like ours.</p>
            <p>Applying the history of DA to the Northern Areas, I argue that we must move beyond Eurocentric models. We need a discourse analysis that understands the "oral history" of the North—where knowledge was passed down through spoken word rather than written archives, requiring a unique historical lens to decode our past.</p>
        `
    },
    {
        id: 5,
        title: "Discourse, Society, and Communities: The Jirga Logic",
        image: "social_gathering_1766764042186.png",
        content: `
            <p>In the North, communities are often governed by the "Jirga" or village councils—a perfect example of communal discourse in action. Here, language is used to maintain harmony and resolve conflict. This entry focuses on the discourse of "The Gathering." In these communities, the speech is collective, not individualistic.</p>
            <p>I analyze how the "Mountain Community" uses specific linguistic markers to reinforce solidarity. Whether it is a collective prayer for the harvest or a communal decision on water rights, the discourse is saturated with the values of "Khandani" (honor) and "Ittehad" (unity). This section examines how the linguistic structures of our valleys create a sense of safety and shared destiny in a harsh environment.</p>
        `
    },
    {
        id: 6,
        title: "Pragmatics: The Unspoken Codes of the North",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>Pragmatics is the study of "invisible" meaning, and in the Northern Areas, what is unsaid is often as important as what is said. In our mountain cultures, meaning is deeply context-dependent. For instance, an invitation to tea in a Balti household carries different pragmatic weight depending on the time of day and the guest's social status.</p>
            <p>This entry examines how speakers in the North use situational cues to communicate. We don't just use language to exchange information; we use it to navigate social landscapes. Understanding the pragmatics of our regional languages reveals the sophisticated logic behind our hospitality and the subtle ways we negotiate our daily lives.</p>
        `
    },
    {
        id: 7,
        title: "Conversational Implicatures: Reading Between the Ridges",
        image: "conversational_implicatures_1767164537987.jpg",
        content: `
            <p>Before studying pragmatics, I believed that meaning existed only in what was spoken. Conversational implicatures taught me that meaning often lies in what is left unsaid. This is especially visible in the communicative culture of Northern Pakistan, particularly in regions like Chitral.</p>
            <p>In many northern communities, direct refusal is considered impolite. Instead of saying “no,” speakers rely on pauses, softened expressions, or indirect responses. Understanding these implicatures requires shared cultural knowledge. Outsiders may miss the meaning, but locals understand immediately.</p>
            <p>Discourse Studies helped me see silence, tone, and hesitation as meaningful elements of conversation. In multilingual settings, implicatures become even more important because speakers carefully choose what to imply rather than state openly. Language in the north is not loud; it is thoughtful, respectful, and deeply contextual.</p>
        `
    },
    {
        id: 8,
        title: "Politeness, Face, and Discourse: The Honor of the Valley",
        image: "social_gathering_1766764042186.png",
        content: `
            <p>In our communities, "Face" (self-esteem and public image) is a sacred currency. Politeness strategies in languages like Shina or Burushaski are highly developed to protect the "Face" of others. Whether it is using specific honorifics for elders or the indirect refusal of a request, our discourse is a shield for dignity.</p>
            <p>This entry analyzes the "Positive" and "Negative" politeness used in Northern social circles. I argue that our discourse is not just about being "nice"; it is a complex linguistic system designed to prevent conflict and preserve the "Khandani" (familial) honor that is the bedrock of mountain society.</p>
        `
    },
    {
        id: 9,
        title: "Multimodality: The Visual Language of the Peaks",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>Discourse isn't limited to speech; it is multimodal. In the North, the vibrant patterns of a Hunza cap, the architecture of a wooden mosque in Skardu, and the calligraphy on roadside trucks all communicate meaning. This section examines how we combine visual symbols with multilingual text to project our identity.</p>
            <p>By looking at local handicrafts and digital media from the region, I analyze how "visual discourse" complements our spoken words. Multimodality allows the North to speak to the world through color, texture, and symbol, creating a rich, sensory narrative that words alone cannot fully capture.</p>
        `
    },
    {
        id: 10,
        title: "Genre Analysis: The Art of the Mountain Folk Tale",
        image: "social_gathering_1766764042186.png",
        content: `
            <p>Genre Analysis helps us understand the conventional structures of specific text types. In the Northern Areas, the "Folk Tale" (Dastan) is a distinct genre with its own linguistic rules and structural expectations. These stories often begin with specific markers and follow a predictable path of heroics and morality.</p>
            <p>In this writing, I break down the genre of Northern storytelling. By analyzing the "moves" within a traditional narrative, I show how these stories function as educational tools. This genre doesn't just entertain; it archives our history and reinforces the linguistic patterns that define our cultural "speech community."</p>
        `
    },
    {
        id: 11,
        title: "Critical Discourse Analysis (CDA): Power and the Peaks",
        image: "cda_map_1767164679228.jpg",
        content: `
            <p>Studying Critical Discourse Analysis made me aware that language is never neutral. Every word choice reflects power, hierarchy, and authority. When I think about the Northern Areas of Pakistan, especially regions like Gilgit-Baltistan, I notice how power operates quietly through language.</p>
            <p>In official documents, education systems, and media, Urdu and English dominate. Local languages such as Shina, Burushaski, and Balti are rarely visible in formal spaces. CDA helped me understand that this absence is also a form of discourse. When a language is excluded, the identity of its speakers is pushed to the margins.</p>
            <p>Through this course, I learned to question who gets to speak and whose voices are heard. The people of the north continue to resist this imbalance by preserving their languages in homes, storytelling, and community gatherings. CDA allowed me to see multilingualism not just as diversity, but as a site of power and struggle.</p>
        `
    },
    {
        id: 12,
        title: "Code-Switching as Social Strategy",
        image: "multilingual_road_1766764060824.png",
        content: `
            <p>In the multilingual valleys of the North, "Code-Switching" is the ultimate discourse tool. It is common to hear a sentence start in Shina, pivot to Urdu for technical terms, and end in English for emphasis. This isn't a sign of linguistic weakness, but of "communicative competence."</p>
            <p>This entry analyzes how Northern speakers strategically switch languages to signal intimacy, authority, or modernity. By examining these shifts, we see how the "discourse of the borderland" is fluid. We navigate multiple worlds in a single breath, using our multilingual repertoire to bridge the gap between our mountain traditions and the globalized future.</p>
        `
    },
    {
        id: 13,
        title: "The Linguistic Landscape of the North",
        image: "multilingual_road_1766764060824.png",
        content: `
            <p>The "Linguistic Landscape" refers to the visibility of languages in public spaces—signs, billboards, and graffiti. In cities like Gilgit or Aliabad, the landscape is a battlefield of discourse. English dominates high-end tourism signs, Urdu prevails in government buildings, and local scripts (like Balti or Arabic-style Shina) appear in religious or cultural spaces.</p>
            <p>This writing examines how these signs tell a story of power and presence. By "reading" the streets of the North, we see which languages are being promoted and which are being silenced, revealing the sociolinguistic priorities of our changing society.</p>
        `
    },
    {
        id: 14,
        title: "Narrative Discourse: The Oral Archives",
        image: "social_gathering_1766764042186.png",
        content: `
            <p>Narrative Discourse is how we structure stories to make sense of our history. In Northern Pakistan, where oral tradition has outlived written records for centuries, the "Narrative" is our primary archive. This final entry explores how the structure of Northern storytelling differs from Western linear models.</p>
            <p>Our narratives are often circular, deeply embedded in the "discourse of place" and spiritual connection to the land. By studying these oral structures, we find that the "History of the North" isn't found in a library; it is found in the living discourse of our elders, preserved through the specific linguistic choices of their storytelling.</p>
        `
    }
];

const creativeNonFictionWritings = [
    {
        id: 1,
        title: "The New Routine",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>The first few days at the new university felt like wearing a pair of shoes that were a size too small. Everything was functional, but nothing felt quite right. The hallways are long and painted a sterile white that reminds me of a doctor’s office, and the stairs are steep and tiring. Looking around, I didn't see a "campus"; I saw a construction project. Piles of bricks and the constant hum of machinery took the place of the green trees and quiet corners I had imagined.</p>

            <p>But as the week went on, I found my spots. I found the corner of the library where the sun hits the desk at exactly 2:00 PM. I found the cafeteria bench where the tea is actually hot. Most importantly, I found that my friends make the grey walls look brighter. We complain about the dust and the long walks together, and in that complaining, we’ve made this place ours. It isn't a beautiful building yet, but it’s becoming a home because we are in it.</p>
        `
    },
    {
        id: 2,
        title: "Why I Put Pen to Paper",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>I don't write because I want to be famous or because I have all the answers. I write because my head gets too crowded. Sometimes, a day feels so full of small moments—a joke shared with a stranger, a sad song on the radio, the way the sky looked before it rained—that I feel like I might burst if I don't write them down.</p>

            <p>Writing is my way of catching things before they fall. If I don't record how I felt on a Tuesday afternoon, that feeling disappears forever. I use simple words because those are the ones that feel true. I write to understand myself, to figure out why I’m happy or why I’m anxious. It’s like talking to a friend who never interrupts and always listens.</p>
        `
    },
    {
        id: 3,
        title: "The \"Yes\" Trap",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>For a long time, I thought being helpful meant never saying "no." If a teacher needed a volunteer, my hand went up. If a friend needed a favor, I was there. I was the person everyone could count on, and I liked that reputation. I thought that by doing everything, I was becoming a better person.</p>

            <p>But eventually, I stopped enjoying the things I loved. Hosting events became a chore. Helping others felt like a heavy weight. I realized that by saying "yes" to everyone else, I was saying "no" to myself. I learned the hard way that you have to save some energy for your own life. Now, I’m learning that "no" is a healthy word. It doesn't mean I’m lazy; it means I’m taking care of my own heart so I have something left to give later.</p>
        `
    },
    {
        id: 4,
        title: "A Campus Observation: What We Fear",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>We decided to walk around the university today and ask people a simple question: "What are you afraid of?" We expected people to talk about big things, like spiders or heights. Instead, the answers were much closer to home.</p>

            <p>One student near the juice stall said she was afraid of failing her midterms because she didn't want to let her parents down.</p>
            <p>A group of boys laughing near the gate grew quiet when we asked. One admitted he was afraid of graduating and not finding a job.</p>
            <p>A girl sitting alone in the courtyard told us she was afraid of being forgotten by her friends back home.</p>

            <p>It made me realize that we are all walking around with these little clouds of worry over our heads. We look confident on the outside, but inside, we are all just trying to make it through.</p>
        `
    },
    {
        id: 5,
        title: "Small Town, Big Dreams",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>I grew up in a place where everyone knew everyone’s business. It was a town of dusty streets and predictable afternoons. Being the oldest child, I was the one who had to set the example. I had to be the responsible one, the one who didn't make mistakes.</p>

            <p>Coming to the city for university was like stepping into a different world. At first, the noise was too loud and the pace was too fast. But I realized that the language I studied gave me a bridge between my quiet hometown and this busy new life. I’m still that girl from the small town, but now I’ve found a voice that can be heard even in a crowded room.</p>
        `
    },
    {
        id: 6,
        title: "The Advice We Carry",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>Don't stay out too late; make sure your clothes are ironed; don't speak until you're spoken to; learn how to make the perfect cup of tea; always smile even when you're tired; don't be too loud or people will think you're rude; save your money; keep your room tidy; don't trust strangers too easily; remember where you came from.</p>

            <p>These are the sentences that play in my head like a background song. They aren't written in any book, but I’ve heard them a thousand times from my mother, my aunts, and my teachers. They are meant to protect me, but sometimes they feel like a fence. I’m learning which rules to keep and which ones are okay to break as I grow up.</p>
        `
    },
    {
        id: 7,
        title: "The Weight of a Phone Call",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>There is a specific kind of silence that follows a phone call with home. I hang up, and for a few minutes, the hostel room feels much smaller than it did before. On the screen, it says "Call Ended: 15 minutes," but the sound of my mother’s voice and the background noise of my siblings laughing stays in the room with me.</p>

            <p>Being away at university is supposed to be about independence, but sometimes independence feels a lot like being untethered. You realize that you are now the person responsible for your own laundry, your own meals, and your own moods. I find myself missing the small, annoying things—the way the gate creaks back home or the way my dad asks the same question every evening. I’m learning that moving forward doesn't mean leaving everything behind; it just means carrying those voices in your pocket while you try to build a life of your own.</p>
        `
    },
    {
        id: 8,
        title: "The Art of Doing Nothing",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>In a world that celebrates "the hustle," I am starting to find beauty in the gaps. We are taught that every hour must be productive—that if we aren't studying, we should be networking, and if we aren't networking, we should be "improving" ourselves. But some of my best thoughts have come when I was doing absolutely nothing.</p>

            <p>I think of the moments spent sitting on the campus grass, watching the shadows of the trees grow longer. Or the minutes spent staring out the window of a moving bus. We need these empty spaces. Without them, life is just a long list of tasks. I’ve realized that my mind needs time to breathe, just like my lungs do. Resting isn't "wasting time"; it’s the time where I actually get to be myself, away from the pressure of being a student or a leader.</p>
        `
    },
    {
        id: 9,
        title: "The Faces in the Hallway",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>I’ve started playing a game during the ten-minute break between classes. I watch the faces of people passing by and try to imagine one thing they are happy about today.</p>

            <p>There is the guy who is walking a little faster than everyone else, probably heading to meet someone he likes. There is the girl leaning against the pillar, eyes closed for a second, probably relieved that a hard presentation is finally over. We all share this space, but we are all in different chapters of our lives. It’s a strange, comforting thought. You are never truly alone in your stress or your joy; there is always someone else in the same hallway feeling something just as deeply as you are.</p>
        `
    },
    {
        id: 10,
        title: "Learning to Let Go of \"Perfect\"",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>I used to be terrified of making a mistake in public. Whether it was a slip of the tongue while hosting an event or a wrong answer in a seminar, I would replay the moment in my head for days. I wanted to be the version of myself that was always polished, always "on."</p>

            <p>But lately, I’ve found that people actually connect more with your mess than your perfection. When I trip over a word and laugh at myself, the audience relaxes. When I admit to a friend that I’m struggling, they usually breathe a sigh of relief and say, "Me too." I am learning that the cracks are where the light gets in. I’m trying to be "real" instead of "perfect." It’s much less exhausting, and honestly, it’s much more fun.</p>
        `
    },
    {
        id: 11,
        title: "The Rain on the Tin Roof",
        image: "visual_artifacts_1766764076674.png",
        content: `
            <p>There is a certain mood that comes with a rainy afternoon on campus. The construction site turns into a landscape of mud, and the sterile white walls of the building finally seem to soften. Everyone huddles under the small awnings, sharing umbrellas and space.</p>

            <p>Rain has a way of slowing everything down. It forces us to stop running. In those moments, the "crisis" of the new campus doesn't matter as much. The deadlines feel a little further away. We stand there, listening to the rhythm of the water, reminded that nature doesn't care about our schedules. It’s a quiet reminder to just exist for a moment, to feel the cool air, and to be thankful for a roof over our heads, no matter how uninspiring that roof might be.</p>
        `
    }
];
